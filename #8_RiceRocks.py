...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            current_explosion_index = self.age
...
def process_sprite_group(rock_set, canvas):
    rock_set_copy = set(rock_set)
    for rock in rock_set_copy:
        if rock.update():
...
def group_group_collide(first_set, second_set):
    collides_counter = 0
    first_set_copy = set(first_set)
    for item in first_set_copy:
        if group_collide(second_set, item):
            collides_counter += 1
            first_set.discard(item)
    return collides_counter
...
def rock_spawner():
    ...
    if len(rock_group) <=12:
        ...
        rock_vel = [random.random() * .4 * score - .3, random.random() * .4 * score - .3]
